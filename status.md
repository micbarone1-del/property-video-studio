...
**Integration test (Suite C) — in progress as of this update, NOT yet fully confirmed.** A disposable 3-scene test job was created and generated successfully; a batch redo of 2 of those 3 scenes (with a caption-only edit to the 3rd, and a deliberately-invalid bogus `scene_id` mixed into the redo list) was submitted and is still running as of this write-up. Do not treat the batch-redo migration as verified until this run completes and each check (clips actually regenerated for the 2 redone scenes only, the 3rd scene's clip untouched but its caption persisted, exactly one combined cost entry with correct tier-aware math, final video reassembled, job lock released cleanly, bogus scene_id skipped without crashing the batch) has actually passed.
